return {
  -- Neotest: test runner framework
  {
    ...
    keys = {
      { '<leader>tt', function() require('neotest').run.run() end, desc = '[T]est: Run nearest [T]est' },
      { '<leader>tf', function() require('neotest').run.run(vim.fn.expand '%') end, desc = '[T]est: Run [F]ile' },
      { '<leader>ta', function() require('neotest').run.run(vim.fn.getcwd()) end, desc = '[T]est: Run [A]ll' },
      ...
      { '<leader>tw', function() require('neotest').watch.toggle(vim.fn.expand '%') end, desc = '[T]est: Toggle [W]atch mode' },
    ...
    keys = {
      ...
      { '<leader>tC', '<cmd>CoverageSummary<cr>', desc = '[T]est: [C]overage summary' },
      {
        ...
        function()
          ...
          pcall(function() require('coverage-r.navigation').clear_cache() end)
          vim.system(
            ...
            function(result)
              vim.schedule(function()
                if result.code == 0 then
                  ...
                  pcall(function() require('coverage').load(true) end)
                else
      ...
      { ']u', function() require('coverage-r.navigation').next_uncovered() end, desc = 'Next uncovered line' },
      { '[u', function() require('coverage-r.navigation').prev_uncovered() end, desc = 'Previous uncovered line' },
      { '<leader>tu', function() require('coverage-r.picker').pick() end, desc = '[T]est: [U]ncovered functions' },
    },
    opts = {
      auto_reload = true,
      lang = {
